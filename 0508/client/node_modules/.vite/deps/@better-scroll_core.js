//#region node_modules/@better-scroll/core/dist/core.esm.js
/*!
* better-scroll / core
...
var isIOSBadVersion = (function() {
	if (typeof ua === "string") {
		var matches = /os (\d\d?_\d(_\d)?)/.exec(ua);
...
if (inBrowser) {
	...
	try {
		var opts = {};
		Object.defineProperty(opts, "passive", { get: function() {
...
var vendor = (function() {
	...
	for (var _i = 0, transformNames_1 = transformNames; _i < transformNames_1.length; _i++) {
		...
		if (elementStyle[obj.value] !== void 0) return obj.key;
	}
	/* istanbul ignore next  */
	return false;
})();
...
function prefixStyle(style) {
	if (vendor === false) return style;
	if (vendor === "standard") {
		if (style === "transitionEnd") return "transitionend";
		return style;
	}
...
function removeEvent(el, type, fn, capture) {
	el.removeEventListener(type, fn, { capture: !!capture });
}
...
function click(e, event) {
	...
	var eventSource;
	if (e.type === "mouseup") eventSource = e;
	else if (e.type === "touchend" || e.type === "touchcancel") eventSource = e.changedTouches[0];
	var posSrc = {};
	if (eventSource) {
	...
	if (typeof MouseEvent !== "undefined") try {
...
var getProperty = function(obj, key) {
	...
	if (typeof obj[lastKey] === "function") return function() {
		return obj[lastKey].apply(obj, arguments);
	};
	else return obj[lastKey];
};
...
var EventEmitter = function() {
	...
	EventEmitter.prototype.off = function(type, fn) {
		...
		if (type) {
			...
			while (count--) if (events[count][0] === fn || events[count][0] && events[count][0].fn === fn) events.splice(count, 1);
			return this;
		}
	};
	...
	EventEmitter.prototype.hasType = function(type) {
		...
		if (!(types[type] === type)) warn("EventEmitter has used unknown event type: \"" + type + "\", should be oneof [" + ("" + Object.keys(types).map(function(_) {
...
var OptionsConstructor = function(_super) {
	...
	function OptionsConstructor() {
		...
		_this.preventDefaultException = { tagName: /^(INPUT|TEXTAREA|BUTTON|SELECT|AUDIO)$/ };
		_this.tagException = { tagName: /^TEXTAREA$/ };
	...
	OptionsConstructor.prototype.resolveBounce = function(bounceOptions) {
		...
		var ret;
		if (typeof bounceOptions === "object") ret = extend(DEFAULT_BOUNCE, bounceOptions);
		else ret = bounceOptions ? DEFAULT_BOUNCE : NEGATED_BOUNCE;
		return ret;
	};
...
var ActionsHandler = function() {
	...
	ActionsHandler.prototype.start = function(e) {
		...
		if (_eventType === 2 && e.button !== 0) return;
		if (this.hooks.trigger(this.hooks.eventTypes.beforeStart, e)) return;
	...
	ActionsHandler.prototype.move = function(e) {
		if (eventTypeMap[e.type] !== this.initiated) return;
		...
		if (pX > document.documentElement.clientWidth - autoEndDistance || pY > document.documentElement.clientHeight - autoEndDistance || pX < autoEndDistance || pY < autoEndDistance) this.end(e);
	};
...
var Translater = function() {
	...
	Translater.prototype.getComputedPosition = function() {
		var matrix = window.getComputedStyle(this.content, null)[style.transform].split(")")[0].split(", ");
		return {
...
var Transition = function(_super) {
	...
	Transition.prototype.startProbe = function(startPoint, endPoint) {
		...
		var probe = function() {
			...
			if (isValidPostion(startPoint, endPoint, pos, prePos)) _this.hooks.trigger(_this.hooks.eventTypes.move, pos);
			/* istanbul ignore if  */
			if (!_this.pending) if (_this.callStopWhenPending) _this.callStopWhenPending = false;
			else _this.hooks.trigger(_this.hooks.eventTypes.end, pos);
			prePos = pos;
			if (_this.pending) _this.timer = requestAnimationFrame(probe);
		};
		if (this.callStopWhenPending) this.setCallStop(false);
		cancelAnimationFrame(this.timer);
		probe();
	};
	...
	Transition.prototype.move = function(startPoint, endPoint, time, easingFn) {
		...
		if (time && isRealtimeProbeType) this.startProbe(startPoint, endPoint);
		if (!time) {
			this._reflow = this.content.offsetHeight;
			if (isRealtimeProbeType) this.hooks.trigger(this.hooks.eventTypes.move, endPoint);
	...
	Transition.prototype.stop = function() {
		if (this.doStop()) this.hooks.trigger(this.hooks.eventTypes.callStop);
	};
	return Transition;
}(Base);
var Animation = function(_super) {
	...
	Animation.prototype.move = function(startPoint, endPoint, time, easingFn) {
		if (!time) {
			this.translate(endPoint);
			if (this.options.probeType === 3) this.hooks.trigger(this.hooks.eventTypes.move, endPoint);
			this.hooks.trigger(this.hooks.eventTypes.end, endPoint);
			return;
		}
	...
	Animation.prototype.animate = function(startPoint, endPoint, duration, easingFn) {
		...
		var step = function() {
			...
			if (now >= destTime) {
				...
				if (isRealtimeProbeType) _this.hooks.trigger(_this.hooks.eventTypes.move, endPoint);
			...
			Object.keys(endPoint).forEach(function(key) {
				...
				newPoint[key] = (endPoint[key] - startValue) * easing + startValue;
			});
			_this.translate(newPoint);
			if (isRealtimeProbeType) _this.hooks.trigger(_this.hooks.eventTypes.move, newPoint);
			if (_this.pending) _this.timer = requestAnimationFrame(step);
			/* istanbul ignore if  */
			if (!_this.pending) if (_this.callStopWhenPending) _this.callStopWhenPending = false;
			else _this.hooks.trigger(_this.hooks.eventTypes.end, endPoint);
		};
...
function createAnimater(element, translater, options) {
	...
	if (useTransition) return new Transition(element, translater, animaterOptions);
	else return new Animation(element, translater, animaterOptions);
}
var Behavior = function() {
	...
	Behavior.prototype.performDampingAlgorithm = function(delta, dampingFactor) {
		...
		if (newPos > this.minScrollPos || newPos < this.maxScrollPos) if (newPos > this.minScrollPos && this.options.bounces[0] || newPos < this.maxScrollPos && this.options.bounces[1]) newPos = this.currentPos + delta * dampingFactor;
		else newPos = newPos > this.minScrollPos ? this.minScrollPos : this.maxScrollPos;
		return newPos;
	};
	Behavior.prototype.end = function(duration) {
		var momentumInfo = { duration: 0 };
		...
		} else this.hooks.trigger(this.hooks.eventTypes.end, momentumInfo);
		return momentumInfo;
	};
	...
	Behavior.prototype.refresh = function(content) {
		...
		/* istanbul ignore if  */
		if (isWrapperStatic) this.relativeOffset -= wrapperRect[position];
	...
	Behavior.prototype.computeBoundary = function() {
		...
		if (boundary.maxScrollPos < 0) {
			...
			if (this.options.specifiedIndexAsContent === 0) boundary.minScrollPos = -this.relativeOffset;
		}
	...
	Behavior.prototype.adjustPosition = function(pos) {
		if (!this.hasScroll && !this.hooks.trigger(this.hooks.eventTypes.ignoreHasScroll)) pos = this.minScrollPos;
		else if (pos > this.minScrollPos) pos = this.minScrollPos;
		else if (pos < this.maxScrollPos) pos = this.maxScrollPos;
		return pos;
	};
...
var DirectionLockAction = function() {
	...
	DirectionLockAction.prototype.adjustDelta = function(deltaX, deltaY) {
		if (this.directionLocked === "horizontal") deltaY = 0;
		else if (this.directionLocked === "vertical") deltaX = 0;
		return {
			deltaX,
			deltaY
		};
	...
	DirectionLockAction.prototype.computeDirectionLock = function(absDistX, absDistY) {
		if (this.directionLocked === "" && !this.freeScroll) if (absDistX > absDistY + this.directionLockThreshold) this.directionLocked = "horizontal";
		else if (absDistY >= absDistX + this.directionLockThreshold) this.directionLocked = "vertical";
		else this.directionLocked = "none";
	};
	DirectionLockAction.prototype.handleEventPassthrough = function(e) {
		var handleMap = DirectionMap[this.directionLocked];
		if (handleMap) {
			if (this.eventPassthrough === handleMap["yes"]) return PassthroughHandlers["yes"](e);
			else if (this.eventPassthrough === handleMap["no"]) return PassthroughHandlers["no"](e);
		}
		return false;
	};
...
var applyQuadrantTransformation = function(deltaX, deltaY, quadrant) {
	if (quadrant === 2) return [deltaY, -deltaX];
	else if (quadrant === 3) return [-deltaX, -deltaY];
	else if (quadrant === 4) return [-deltaY, deltaX];
	else return [deltaX, deltaY];
};
var ScrollerActions = function() {
	...
	ScrollerActions.prototype.bindActionsHandler = function() {
		...
		this.actionsHandler.hooks.on(this.actionsHandler.hooks.eventTypes.click, function(e) {
			if (_this.enabled && !e._constructed) _this.handleClick(e);
		});
	};
	...
	ScrollerActions.prototype.handleMove = function(deltaX, deltaY, e) {
		if (this.hooks.trigger(this.hooks.eventTypes.beforeMove, e)) return;
		var absDistX = this.scrollBehaviorX.getAbsDist(deltaX);
		var absDistY = this.scrollBehaviorY.getAbsDist(deltaY);
		var timestamp = getNow();
		if (this.checkMomentum(absDistX, absDistY, timestamp)) return true;
		...
		if (this.hooks.trigger(this.hooks.eventTypes.detectMovingDirection)) return;
		if (!this.fingerMoved) this.fingerMoved = true;
		var positionChanged = newX !== prevX || newY !== prevY;
		if (!this.contentMoved && !positionChanged) this.hooks.trigger(this.hooks.eventTypes.contentNotMoved);
	...
	ScrollerActions.prototype.handleEnd = function(e) {
		if (this.hooks.trigger(this.hooks.eventTypes.beforeEnd, e)) return;
		var currentPos = this.getCurrentPos();
		this.scrollBehaviorX.updateDirection();
		this.scrollBehaviorY.updateDirection();
		if (this.hooks.trigger(this.hooks.eventTypes.end, e, currentPos)) return true;
...
function bubbling(source, target, events) {
	events.forEach(function(event) {
		var sourceEvent;
		var targetEvent;
		if (typeof event === "string") sourceEvent = targetEvent = event;
		else {
...
var Scroller = function() {
	...
	Scroller.prototype.bindTranslater = function() {
		...
		hooks.on(hooks.eventTypes.beforeTranslate, function(transformStyle) {
			if (_this.options.translateZ) transformStyle.push(_this.options.translateZ);
		});
		hooks.on(hooks.eventTypes.translate, function(pos) {
			...
			if (pos.x !== prevPos.x || pos.y !== prevPos.y) _this.togglePointerEvents(false);
		});
	};
	...
	Scroller.prototype.bindActions = function() {
		...
		actions.hooks.on(actions.hooks.eventTypes.end, function(e, pos) {
			...
			if (_this.hooks.trigger(_this.hooks.eventTypes.end, pos)) return true;
			if (!actions.fingerMoved) {
				_this.hooks.trigger(_this.hooks.eventTypes.scrollCancel);
				if (_this.checkClick(e)) return true;
			}
		...
		actions.hooks.on(actions.hooks.eventTypes.scrollEnd, function(pos, duration) {
			...
			if (actions.contentMoved) _this.hooks.trigger(_this.hooks.eventTypes.scrollEnd, pos);
			if (_this.animater.forceStopped) _this.animater.setForceStopped(false);
		});
	};
	Scroller.prototype.checkFlick = function(duration, deltaX, deltaY) {
		var flickMinMovingDistance = 1;
		if (this.hooks.events.flick.length > 1 && duration < this.options.flickLimitTime && deltaX < this.options.flickLimitDistance && deltaY < this.options.flickLimitDistance && (deltaY > flickMinMovingDistance || deltaX > flickMinMovingDistance)) return true;
	};
	Scroller.prototype.momentum = function(pos, duration) {
		...
		if (meta.newX !== pos.x || meta.newY !== pos.y) {
			if (meta.newX > this.scrollBehaviorX.minScrollPos || meta.newX < this.scrollBehaviorX.maxScrollPos || meta.newY > this.scrollBehaviorY.minScrollPos || meta.newY < this.scrollBehaviorY.maxScrollPos) meta.easing = ease.swipeBounce;
			this.scrollTo(meta.newX, meta.newY, meta.time, meta.easing);
			return true;
		}
	};
	Scroller.prototype.checkClick = function(e) {
		var cancelable = { preventClick: this.animater.forceStopped };
		...
		if (!cancelable.preventClick) {
			...
			if (this.options.tap) tap(e, this.options.tap);
			if (this.options.click && !preventDefaultExceptionFn(e.target, this.options.preventDefaultException)) click(e);
	...
	Scroller.prototype.resize = function() {
		...
		/* istanbul ignore if  */
		if (isAndroid) this.wrapper.scrollTop = 0;
	...
	Scroller.prototype.transitionEnd = function(e) {
		if (e.target !== this.content || !this.animater.pending) return;
		this.animater.transitionTime();
		if (!this.resetPosition(this.options.bounceTime, ease.bounce)) {
			this.animater.setPending(false);
			if (this.options.probeType !== 3) this.hooks.trigger(this.hooks.eventTypes.scrollEnd, this.getCurrentPos());
		}
	};
	Scroller.prototype.togglePointerEvents = function(enabled) {
		...
		for (var i = 0; i < el.length; i++) {
			var node = el[i];
			/* istanbul ignore if  */
			if (node.isBScrollContainer) continue;
	...
	Scroller.prototype.refresh = function(content) {
		...
		if (contentChanged) {
			...
			this.registerTransitionEnd();
			if (this.options.bindToTarget) this.actionsHandler.setContent(content);
		}
	...
	Scroller.prototype.setContent = function(content) {
		...
		if (contentChanged) this.content = content;
	...
	Scroller.prototype.scrollToElement = function(el, time, offsetX, offsetY, easing) {
		...
		if (this.hooks.trigger(this.hooks.eventTypes.scrollToElement, targetEle, pos)) return;
	...
	Scroller.prototype.resetPosition = function(time, easing) {
		...
		if (xInBoundary && yInBoundary) return false;
		/* istanbul ignore if  */
		if (isIOSBadVersion) this.reflow();
...
var BScrollConstructor = function(_super) {
	...
	function BScrollConstructor(el, options) {
		...
		if (!_this.setContent(wrapper).valid) return _this;
	...
	BScrollConstructor.use = function(ctor) {
		...
		if (BScrollConstructor.plugins.some(function(plugin) {
	...
	BScrollConstructor.prototype.init = function(wrapper) {
		...
		if (this.hooks.trigger(this.hooks.eventTypes.beforeInitialScrollTo, position)) return;
	...
	BScrollConstructor.prototype.applyPlugins = function() {
		...
		BScrollConstructor.plugins.sort(function(a, b) {
			var _a;
			var applyOrderMap = (_a = {}, _a["pre"] = -1, _a["post"] = 1, _a);
			return (a.applyOrder ? applyOrderMap[a.applyOrder] : 0) - (b.applyOrder ? applyOrderMap[b.applyOrder] : 0);
		}).forEach(function(item) {
			var ctor = item.ctor;
			if (options[item.name] && typeof ctor === "function") _this.plugins[item.name] = new ctor(_this);
		});
	};
	BScrollConstructor.prototype.handleAutoBlur = function() {
		/* istanbul ignore if  */
		if (this.options.autoBlur) this.on(this.eventTypes.beforeScrollStart, function() {
			var activeElement = document.activeElement;
			if (activeElement && (activeElement.tagName === "INPUT" || activeElement.tagName === "TEXTAREA")) activeElement.blur();
		});
	};
	...
	BScrollConstructor.prototype.refresh = function() {
		var _a = this.setContent(this.wrapper), contentChanged = _a.contentChanged;
		if (_a.valid) {
...
function createBScroll(el, options) {
	return new BScrollConstructor(el, options);
}
